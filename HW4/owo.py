...
def record1(seconds, test_y, epochs, callback):
...
def record2(seconds, test_y, epochs, callback):
	with open('test{seconds}.txt'.format(seconds=seconds), 'w') as file:
		file.write('id,good,bad\n')
...
if __name__ == '__main__':
	...
	model = Conv1D(name="conv1D_good", filters=64, kernel_size=6, kernel_initializer=initializers.glorot_uniform(seed=1))(model)
	model = BatchNormalization(name="batchNormalization_good", epsilon=.000001, momentum=.5)(model)

	good = MaxPooling1D(name="maxPooling1D_good", pool_size=3, strides=1)(model)
	good = Flatten(name="flatten_good")(good)
	good = Dense(name="dense_good_1", output_dim=128, kernel_initializer=initializers.glorot_uniform(seed=1), activation='relu')(good)
	...
	bad = Lambda(name="lambda_bad", function=lambda x: -x)(model)
	bad = MaxPooling1D(name="maxPooling1D_bad", pool_size=3, strides=1)(bad)
	bad = Flatten(name="flatten_bad")(bad)
	bad = Dense(name="dense_bad_1", output_dim=128, kernel_initializer=initializers.glorot_uniform(seed=1), activation='relu')(bad)
	...
	record2(seconds, test_y, epochs, callback)
