...
def dis(input_shape):
    ...
    x = Conv2D(filters=4, kernel_size=(4, 4), strides=(2, 2), kernel_initializer=glorot_uniform(seed=1), activation=LeakyRelu(alpha=.2))(x)
    x = Conv2D(filters=8, kernel_size=(4, 4), strides=(2, 2), kernel_initializer=glorot_uniform(seed=1), activation=LeakyRelu(alpha=.2))(x)
    x = BatchNormalization(epsilon=.000001, momentum=.5)(x)
    x = Conv2D(filters=16, kernel_size=(4, 4), strides=(2, 2), kernel_initializer=glorot_uniform(seed=1), activation=LeakyRelu(alpha=.2))(x)
    x = BatchNormalization(epsilon=.000001, momentum=.5)(x)
    x = Conv2D(filters=32, kernel_size=(4, 4), strides=(2, 2), kernel_initializer=glorot_uniform(seed=1), activation=LeakyRelu(alpha=.2))(x)
    x = BatchNormalization(epsilon=.000001, momentum=.5)(x)
    # 輸出 通常是1維輸出 不過這裡的Example Code 是Patch GAN的Dis 所以你要直接Dense or Conv2D都是可以的
    # Patch gan是說 只要
    # Select 1 Patch-GAN Dis 
    x = Conv2D(filters=1, kernel_size=(4, 4), strides=(1, 1), kernel_initializer=glorot_uniform(seed=1), activation=Adam(lr=0.0002,beta_1=0.5))(x)
    # Select 2 Dense 
...
def gen(input_shape,** kwargs):
    ...
    x4 = conv_block(<your creativity>) #降維step4   # output_size (None,filter_D,D,D)  #然而filter_A<=filter_B<=filter_C<=filter_D
